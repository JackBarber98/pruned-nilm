...
training_chunker = InputChunkSlider(file_name=training_directory, chunk_size=SIZE_OF_CHUNK, batch_size=1000,shuffle=True, offset=window_offset, header=0, ram_threshold=5*10**5)
model = create_model()
model.compile(Adam(learning_rate=0.001, beta_1=0.9, beta_2=0.999), loss="mean_squared_error", metrics=["mae"])
training_history = model.fit_generator(training_chunker.load_dataset(), steps_per_epoch=1000, epochs=5, verbose=2, callbacks=None)
