...
steps_per_epoch = np.round(int(training_chunker.total_size / DEFAULT_BATCH_SIZE), decimals=0)
...
training_history = model.fit_generator(training_chunker.load_dataset(), steps_per_epoch=steps_per_epoch, epochs=10, verbose=2, callbacks=[early_stopping])

# Plot the monitored metrics and save.
plt.plot(training_history.history['loss'], label="Loss")
plt.plot(training_history.history['mae'], label="MAE")
plt.plot(training_history.history['mse'], label="MSE")
