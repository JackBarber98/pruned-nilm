import os
from data_feeder import ChunkSlider
...
training_chunker = ChunkSlider(file_name=training_directory, batch_size=1000, chunk_size=SIZE_OF_CHUNK, crop=None, shuffle=True, offset=window_offset, header=0, ram_threshold=5*10**5)
model = create_model(100)
model.compile(Adam(learning_rate=0.001, beta_1=0.9, beta_2=0.999), loss="mean_squared_error", metrics=["accuracy", "mae"])
